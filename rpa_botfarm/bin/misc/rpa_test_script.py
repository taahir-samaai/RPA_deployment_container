#!/usr/bin/env python3
"""
Simple test script for Evotel validation that shows callbacks
"""
import argparse
import json
import logging
...
logger = logging.getLogger("evotel-test")
...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Process JOB_EVI for Evotel-specific data
            ...
            if job_evi:
                ...
                if isinstance(job_evi, dict):
                    ...
                    # Show Evotel-specific highlights
                    evotel_highlights = [
                        "evotel_customer_name", "evotel_service_provider", "evotel_product",
                        "evotel_service_status", "evotel_customer_email", "evotel_customer_mobile",
                        "evotel_fsan_number", "evotel_verification_status", "evotel_work_order_reference",
                        "evotel_completeness_score"
                    ]
                    
                    # Display highlights first
                    for key in evotel_highlights:
                        if key in job_evi:
                            display_key = key.replace("evotel_", "").replace("_", " ").title()
                            print(f"  🔸 {Colors.YELLOW}{display_key}{Colors.END}: {job_evi[key]}")
                    
                    # Count total fields
                    evotel_fields = [k for k in job_evi.keys() if k.startswith('evotel_')]
                    evidence_fields = [k for k in job_evi.keys() if k.startswith('evidence_')]
                    
                    print(f"\n  📊 Data Summary:")
                    print(f"    - Evotel fields: {len(evotel_fields)}")
...
def run_evotel_validation(orchestrator_url, serial_number):
    """Run an Evotel validation job and monitor for completion and callbacks."""
    start_time = datetime.now()
    
    # Generate test ID
    test_id = str(uuid.uuid4())[:8]
    external_job_id = f"EVOTEL_TEST_{test_id}"
    
    logger.info(f"Starting Evotel validation for serial {serial_number}")
    ...
    completed = False
    max_wait_time = 300  # 5 minutes for Evotel (can be slow)
    ...
    try:
        ...
        if isinstance(result, dict) and "details" in result:
            ...
            if service_summary:
                ...
                print(f"  Email: {service_summary.get('email', 'N/A')}")
                print(f"  Mobile: {service_summary.get('mobile', 'N/A')}")
                print(f"  Address: {service_summary.get('address', 'N/A')}")
            ...
            if technical_details:
                ...
                if ont_details:
                    print(f"\n{Colors.BOLD}Technical Information:{Colors.END}")
                    print(f"  FSAN Number: {ont_details.get('fsan_number', 'N/A')}")
                    print(f"  Verification: {ont_details.get('verification', 'N/A')}")
                    print(f"  Port Number: {ont_details.get('port_number', 'N/A')}")
            
            # Work order summary
            work_order_summary = details.get("work_order_summary", {})
            if work_order_summary:
                print(f"\n{Colors.BOLD}Work Order Information:{Colors.END}")
                print(f"  Total Work Orders: {work_order_summary.get('total_work_orders', 0)}")
                print(f"  Primary Reference: {work_order_summary.get('primary_work_order_reference', 'N/A')}")
                print(f"  Primary Status: {work_order_summary.get('primary_work_order_status', 'N/A')}")
            
            # Data completeness
            extraction_metadata = details.get("extraction_metadata", {})
            if extraction_metadata:
                completeness_score = extraction_metadata.get("completeness_score", 0) * 100
                print(f"\n{Colors.BOLD}Data Quality:{Colors.END}")
                print(f"  Completeness Score: {completeness_score:.1f}%")
                print(f"  Sections Extracted: {extraction_metadata.get('total_sections_extracted', 0)}")
...
def main():
    parser = argparse.ArgumentParser(description="Test Evotel Validation with Callbacks")
    parser.add_argument("--url", default="http://localhost:8620", help="Orchestrator URL")
    parser.add_argument("--serial", default="48575443D9B290B1", help="Serial number to test")
    parser.add_argument("--callback-port", type=int, default=8625, help="Port for callback listener")
    
    args = parser.parse_args()
    
    print(f"{Colors.BOLD}{Colors.BLUE}Starting Evotel Validation Test with Callbacks...{Colors.END}")
    print(f"{Colors.BOLD}Serial Number: {args.serial}{Colors.END}")
    ...
    try:
        success = run_evotel_validation(args.url, args.serial)
